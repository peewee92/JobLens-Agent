...
test("import detail summarizes zero-provider next steps for the imported batch", async () => {
  ...
  assert.match(page, /Requirement 尚未达到 release 条件/);
  assert.match(page, /Evidence action 精确命中 \$\{batchNextAction\.impactCount\} 个仍在考虑岗位/);
  ...
  assert.match(page, /selectBatchNextAction/);
  assert.match(page, /当前主行动：\{batchNextActionPresentation\.label\}/);
  assert.match(page, /current MatchReport/);
  assert.match(page, /Requirement 已达到 release \/ Match readiness/);
  assert.match(page, /Requirement 尚未达到 release 条件/);
  assert.match(page, /Evidence action 精确命中/);
  assert.match(page, /readiness 暂无法可靠确认/);
...
test("recommendations page surfaces the MVP Top-N value without fake probability language", async () => {
  ...
  assert.match(page, /afterEvidence=1/);
  assert.match(page, /focusRequirementId=/);
...
test("import improvement explains why the next evidence action changed", async () => {
  ...
  assert.match(page, /afterPrepareEvidence/);
  ...
  assert.match(page, /这次 Profile Evidence 更新已经形成完整决策结果/);
  assert.match(page, /原计划 Evidence 的直接产出与同次更新中其他 Evidence 的额外价值/);
  assert.match(page, /<span>直接解锁<\/span>/);
  assert.match(page, /<span>额外解锁<\/span>/);
  assert.match(page, /evidenceUpdateDecisionLoopComplete/);
  assert.match(page, /evidenceUpdatePrimaryApplicationReport/);
  assert.match(page, /优先选择 current Ranking 最高的“感兴趣”岗位/);
  assert.match(page, /明确“不考虑”的岗位不会被继续推动/);
  assert.match(page, /prepareEvidenceDecisionQuery\}\$\{alternativeEvidenceDecisionQuery/);
});
