...
import {reviewActionsForRun} from "@/lib/profile-evals";

export function ProfileEvalReviewForm({
  run,
  alreadyReviewed,
}: {
  ...
  const actions = useMemo(
    () => reviewActionsForRun(run, alreadyReviewed),
    [run, alreadyReviewed],
  );
  const [reviewer, setReviewer] = useState("local-user");
  const [notes, setNotes] = useState("");
  const [pending, setPending] = useState<ProfileEvalReviewDecision | null>(null);
  const [error, setError] = useState<string | null>(null);

  async function submit(decision: ProfileEvalReviewDecision) {
    setError(null);
    setPending(decision);
    try {
      const response = await fetch(
        `/api/profile-evals/${encodeURIComponent(run.id)}/review`,
        {
          method: "POST",
    ...
    <div className="review-form">
      ...
      <label>
        ...
        <textarea
          ...
          minLength={10}
          required
          placeholder="记录逐案例与 Trace 审查结论，至少 10 个字符。"
          rows={5}
        />
      </label>
