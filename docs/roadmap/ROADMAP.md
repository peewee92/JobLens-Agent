...
- 2026-08-28：把单岗位改善继续扩成“这次资料更新还影响了哪些岗位”的最小跨岗位反馈，同时收紧因果归因。`match-improvement` 现在只把同一 Job / Profile 身份 / Requirement Extraction 下、但 **Profile version 不同** 的历史 MatchReport 视为 Evidence 更新可比前序；同一 Profile version 的重复 Match 不再被误报为“补资料带来的改善”，API 同时暴露前后 Profile version 供调用方验证。推荐页仅在 `focusJob` 回流场景，对当前已展示的最多 10 个其他岗位并行读取这一只读 delta，列出确实少了 hard blocker 或 recommendation 发生改善的岗位；任一比较读取失败均 fail-soft，不影响当前推荐。该切片不新增 Provider、DB 写入、迁移或自动用户判断。
