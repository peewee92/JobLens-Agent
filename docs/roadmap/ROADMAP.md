...
- MatchReport 正式 DB migration 已完成；20 岗位人工 Match 评审与基于 UserFeedback 的人工基准仍待完成。2026-08-26 对 4 个 v42.95 真实岗位执行 0-Provider Eligibility 后均为 `blocked`。真实数据暴露并修正了一类确定性假阴性：**不带可验证年限的语义型经验要求**（如“有 LLM/Agent 应用落地项目经验”）在没有精确文本命中时不再直接 `missing`，而是 `conditional` 留给后续 Evidence-based Semantic Match；带明确专项年限的要求仍保持 fail-closed。修复后 Harness missing 2→1、金山办公 Agent 3→2、汽车 AI 场景 5→3。当前下一公共数据缺口仍是保存的 UserProfile 没有 `education` Evidence；同时 Education evaluator 还需继续区分学历层级与“相关专业/知名高校”等限定，避免补录本科后产生假阳性。16 个旧 Extraction 岗位则需要在明确 Provider 预算下逐步刷新到冻结 v42.95，而不是绕过版本门禁。
