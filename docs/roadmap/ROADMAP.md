...
- MatchReport 正式 DB migration 已完成；20 岗位人工 Match 评审与基于 UserFeedback 的人工基准仍待完成。2026-08-26 对 4 个 v42.95 真实岗位执行 0-Provider Eligibility 后均为 `blocked`，当前最明显的公共假阴性来源是保存的 UserProfile 没有 `education` Evidence，导致“本科及以上”硬要求被判 `missing`；Profile 编辑器已支持教育 Evidence，因此下一步应先由用户确认/补全真实教育事实，再重跑 Eligibility。16 个旧 Extraction 岗位则需要在明确 Provider 预算下逐步刷新到冻结 v42.95，而不是绕过版本门禁。
