...
class CareerAgentLoopError(ValueError):
    ...
    @classmethod
    def transient_network(cls, tool: CareerAgentToolName) -> "CareerAgentLoopError":
        return cls(
            code=CareerAgentLoopErrorCode.TRANSIENT_NETWORK,
            message=f"transient network failure for Career Agent tool: {tool.value}",
            retryable=True,
        )
