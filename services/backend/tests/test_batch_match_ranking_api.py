...
def _stored(report_id: str, job_id: str, recommendation: MatchRecommendation) -> StoredMatchReport:
    return StoredMatchReport(
        ...
        report=MatchReport(
            ...
            evidence_links=(
                MatchEvidenceLink(
                    requirement_id=f"req_{job_id}",
                    evidence_ids=(f"evidence_{job_id}",),
                ),
            ),
...
class _UseCase:
    ...
    def execute(
        ...
        *,
        ...
        ranked = (
            (*reports, _stored("report_3", "job_3", MatchRecommendation.BLOCKED))
            if include_blocked
            else reports
        )
...
def test_batch_match_ranking_api_returns_ranked_persisted_snapshots() -> None:
    ...
    assert body["count"] == 2
    assert body["items"][0]["evidenceLinks"] == [
        {"requirementId": "req_job_2", "evidenceIds": ["evidence_job_2"]}
    ]
    assert body["dbWrites"] == 0
    assert body["providerCalls"] == 0
    assert body["traceRunsCreated"] == 0
    assert use_case.calls == [(('job_1', 'job_2'), False, None)]
...
def test_batch_match_ranking_api_passes_explicit_include_blocked() -> None:
    ...
    assert use_case.calls == [(('job_1',), True, None)]


def test_batch_match_ranking_api_returns_requested_top_n_only() -> None:
    use_case = _UseCase()
    app.dependency_overrides[get_batch_match_ranking_use_case] = lambda: use_case
    try:
        with TestClient(app) as client:
    ...
    assert response.status_code == 200
    body = response.json()
    assert [item["reportId"] for item in body["items"]] == ["report_2"]
    assert body["count"] == 1
    assert use_case.calls == [(('job_1', 'job_2'), False, 1)]


def test_batch_match_ranking_api_rejects_invalid_top_n() -> None:
    with TestClient(app) as client:
        response = client.get(
            "/api/v1/match-ranking",
            params=[("jobId", "job_1"), ("topN", "0")],
        )
...
def test_openapi_registers_batch_match_ranking_as_read_only_get() -> None:
    ...
    assert parameter_names == {"jobId", "includeBlocked", "topN"}
