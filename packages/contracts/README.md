...
- `user-feedback.schema.json` — 用户对推荐的人类反馈；必须绑定 immutable `matchReportId`，decision 为 `interested / maybe / rejected`，reasons 使用稳定枚举，`rejected` 至少一个 reason，`other` 必须附 note
