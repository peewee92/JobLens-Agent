...
Phase 6：Target Cohort + Skill Gap ✅ ENGINEERING_COMPLETE / REAL_REQUIREMENT_BASELINE_BLOCKED（已出现真实 interested / maybe 岗位，但 current v42.95 Requirement Review 候选被拆成 16 个 deepseek-v4-flash + 4 个 deepseek-v4-pro，尚无可用于 Gap 的 human-accepted baseline）
...
截至 2026-09-03 的真实主循环进度由 `cd services/backend && .venv/bin/python -m scripts.check_mvp_progress --json` 复核：离线 MVP gate 已通过，20 个真实岗位已经全部有 current v42.95 Requirement Extraction；8 个已有 current MatchReport，另外 12 个是 `match-ready-without-report`，`requirement-analysis-needed=0`。由于 current MatchReport 更新后旧 Feedback 不再计入 current-report coverage，当前进度检查为 `1/8` feedback-covered，`next_priority=complete_current_user_feedback`；用户历史真实判断仍保留，且已经出现 `interested / maybe`，因此 Phase 6 的下一个事实门禁转为 Requirement accepted baseline。当前 20 个 Requirement Review candidates 并不能组成正式 20-case Batch：其中 16 个属于 `openai / deepseek-v4-flash / requirement-extractor-v42.95 / requirement-extraction-v7`，另外 4 个仍是 `deepseek-v4-pro`；后端正式证据要求 20 个 Case 的 Provider / Model / Extractor / Prompt 完全一致。Manual Review 页面现在会显式显示 `16/20`、列出 4 个需重新分析的岗位，并在真正达到 20/20 后提供“一键选择正式 20 条”；不会再把“候选总数 20”误表现为“正式证据已准备好”。这些 outlier 现在可以直接在 Manual Review 页面原地处理：每个岗位单独勾选真实 Provider 成本确认后执行“确认成本并重新分析到目标版本”，成功后刷新当前 readiness，不再要求用户在多个岗位详情页之间来回跳；native form 失去 hydration 时也只允许安全返回 Manual Review 页面。单岗位“分析 / 重新分析岗位要求”的 Backend `confirmLiveCost` 门禁保持不变。下一人工步骤是先对这 4 个旧模型岗位逐次确认成本并重新分析，使 20 个 current Extraction 进入同一 Cohort；随后创建 20-case Review Batch，由用户本人逐 Case Review 并最终决定 `accept_for_match / reject_for_match`。自动任务不得代签该人工结论。Phase 8 维持 `ENGINEERING_COMPLETE`，不进入 Multi-Agent 或未经独立质量证据支持的自由文本路由。
